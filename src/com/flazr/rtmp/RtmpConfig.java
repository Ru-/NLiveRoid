...
//Flazrのプロパティを読みとっている
        ...
        if(is != null) {
            Log.d("flazr-init.properties found on classpath","");
            final Properties flazrProps = loadProps(is);
            flazrHome = flazrProps.getProperty("flazr.home");
            if(flazrHome == null) {
                Log.d("'flazr.home' key not found, will try system properties","");
            }
        } else {
            Log.d("flazr-init.properties not found on classpath, will try system property 'flazr.home'","");
        }
        if(flazrHome == null) {
        ...
        if(flazrHome == null) {
            ...
            Log.d("'flazr.home' system property not set, will use current directory: ", currentDir.getAbsolutePath());
            flazrHome = "";
        } else if(!flazrHome.endsWith("/")) {
            flazrHome = flazrHome + "/";
            Log.d("using 'flazr.home' = ", ""+flazrHome);
        }
        File propsFile = new File(flazrHome + "conf/flazr.properties");
        if(!propsFile.exists()) {
            Log.d("propsFile　not found, will use configuration defaults", propsFile.getAbsolutePath());
        } else {
            Log.d("loading config from: ", ""+propsFile.getAbsolutePath());
            ...
            switch(type) {
                ...
                case SERVER_STOP:
                    ...
                    if(!homeFile.exists()) {
                        Log.d("home dir does not exist, aborting: ", homeFile.getAbsolutePath());
                        throw new RuntimeException("home dir does not exist: " + homeFile.getAbsolutePath());
                    }
                    Log.d("home dir: '", homeFile.getAbsolutePath());
                    Log.d("server port:  stop　", ""+SERVER_PORT+" "+ SERVER_STOP_PORT);
                    break;
                case PROXY:
                case PROXY_STOP:
                    ...
                    Log.d("proxy port:  stop ", ""+PROXY_PORT+" "+ PROXY_STOP_PORT);
                    Log.d("proxy remote host:  port: ", ""+PROXY_REMOTE_HOST + " "+ PROXY_REMOTE_PORT);
                    break;
            }
        }
    }
    ...
    private static Integer parseInt(String s) {
        try {
            ...
            Log.d("unable to parse into integer value: ", e.getMessage());
            return null;
        }
    }
