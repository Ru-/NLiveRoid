...
public class ChannelBuffers {
     ...
     */
    public static final ChannelBuffer EMPTY_BUFFER = new BigEndianHeapChannelBuffer(new byte[0],0,0);
    ...
    public static ChannelBuffer buffer(ByteOrder endianness, int capacity) {
        if (endianness == BIG_ENDIAN) {
            ...
            return new BigEndianHeapChannelBuffer(array,0,0);
        } else if (endianness == LITTLE_ENDIAN) {
            if (capacity == 0) {
                return EMPTY_BUFFER;
            }
            byte[] array = new byte[capacity];
            return new LittleEndianHeapChannelBuffer(array,0,0);
        } else {
    ...
    public static ChannelBuffer wrappedBuffer(ByteOrder endianness, byte[] array) {
        if (endianness == BIG_ENDIAN) {
            ...
            return new BigEndianHeapChannelBuffer(array,0,array.length);
        } else if (endianness == LITTLE_ENDIAN) {
            if (array.length == 0) {
                return EMPTY_BUFFER;
            }
            return new LittleEndianHeapChannelBuffer(array,0,array.length);
        } else {
            throw new NullPointerException("endianness");
        }
    }
    ...
    public static ChannelBuffer copiedBuffer(ByteOrder endianness, byte[] array) {
        if (endianness == BIG_ENDIAN) {
            ...
            byte[] array_clone = array.clone();
            return new BigEndianHeapChannelBuffer(array_clone,0,array_clone.length);
        } else if (endianness == LITTLE_ENDIAN) {
            if (array.length == 0) {
                return EMPTY_BUFFER;
            }
            byte[] array_clone = array.clone();
            return new LittleEndianHeapChannelBuffer(array_clone,0,array_clone.length);
        } else {
            throw new NullPointerException("endianness");
        }
    }
    ...
    public static String hexDump(ChannelBuffer buffer, int fromIndex, int length) {
        ...
        for (; srcIdx < endIndex; srcIdx ++, dstIdx += 2) {
            System.arraycopy(
                    HEXDUMP_TABLE, (short) (buffer.getByte(srcIdx)& 0xFF) << 1,
                    buf, dstIdx, 2);
        }
