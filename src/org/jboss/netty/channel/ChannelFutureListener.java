...
public interface ChannelFutureListener extends EventListener {
    ...
    static ChannelFutureListener CLOSE = new ChannelFutureListener() {
        public void operationComplete(ChannelFuture future) {
    		future.setFailure(new Throwable("ネットワークエラーが発生しました:01"));
        	Log.d("ChannelFutureListener","CLOSE_operationComplete");
    ...
    static ChannelFutureListener CLOSE_ON_FAILURE = new ChannelFutureListener() {
        public void operationComplete(ChannelFuture future) {
        	Log.d("ChannelFutureListener","CLOSE_ON_FAILURE_operationComplete");
        	if (!future.isSuccess()) {
        		future.setFailure(new Throwable("ネットワークエラーが発生しました:02"));
                future.getChannel().close();
            }
        }
    };
